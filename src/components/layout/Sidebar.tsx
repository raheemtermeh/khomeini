...
import {
  ...
  IoLogOutOutline,
  IoPricetagOutline,
  IoLocationOutline, // ✨ جدید: برای مدیریت شعب
  IoMegaphoneOutline, // ✨ جدید: برای اطلاع رسانی
  IoLayersOutline, // ✨ جدید: برای مدیریت کافه/موجودی
...
const Sidebar = ({ onLogoutClick, isOpen, onCloseSidebar }: Props) => {
  ...
  const menuItems = [
    ...
    { title: "سفارشات", icon: <BsReceipt size={22} />, path: "/orders" }, // { //  title: "اعلانات", //  icon: <IoNotificationsOutline size={22} />, //  path: "/notifications", // },
    {
      title: "گزارش مالی",
      icon: <IoPieChartOutline size={22} />,
    ...
    {
      // ✨ آیکون مرتبط: IoLocationOutline برای مدیریت مکان‌ها/شعب
      title: "مدیریت شعب",
      icon: <IoLocationOutline size={22} />,
      path: "/cafe",
    },
    {
      // ✨ آیکون مرتبط: IoMegaphoneOutline برای اطلاعیه
      title: "اطلاع رسانی",
      icon: <IoMegaphoneOutline size={22} />,
      path: "/announcements",
    },
    {
      // ✨ آیکون مرتبط: IoLayersOutline برای مدیریت منابع/موجودی
      title: "مدیریت کافه",
  ...
  const avatarContainerClasses = `p-1 rounded-full bg-primary-red shadow-2xl shadow-primary-red/40 transition-transform duration-300 transform hover:scale-[1.05] relative after:content-[''] after:absolute after:inset-0 after:rounded-full after:bg-primary-red after:opacity-50 after:animate-pulse`; // ✨ استایل آواتار در حالت Hover (برای جلوگیری از تداخل با Pulse)
  const avatarImageClasses =
    "w-24 h-24 rounded-full object-cover border-4 border-white dark:border-gray-800 relative z-10";
